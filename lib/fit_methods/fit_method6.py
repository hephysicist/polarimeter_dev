...
class FitMethod6:
    ...
    def __call__(self, *par):  
        ...
        D1C = np.real(np.fft.ifft2(fD1C))
        DC1 = np.real(np.fft.ifft2(fDC1))
        ...
        C1   = self.shrink(C1, self.add_shape)
        D1C = self.shrink(D1C, self.add_shape)
        DC1 = self.shrink(DC1, self.add_shape)



        self.fit_diff        =  DC1
        self.data_diff       =  D1C
        self.data_diff_error =  np.sqrt( np.power(C0-C1, 2.0) * self.data_left/NL/NL + np.power(C0+C1, 2.0) * self.data_right/NR/NR)/10
